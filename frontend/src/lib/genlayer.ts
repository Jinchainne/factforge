...
export const CONTRACT_ADDRESS =
  (import.meta.env.VITE_FACTFORGE_ADDRESS as string) ||
  "0x866fb1b20Ef82195Dee024117AC2C1bDAF03A9e5";

function address(value: string) {
  if (!/^0x[a-fA-F0-9]{40}$/.test(value))
    throw new Error("Set a valid VITE_FACTFORGE_ADDRESS");
...
export async function createChallenge(
  ...
  const hash = await client.writeContract({
    ...
    args: [statement, rules, position, deadline],
    value: stakeWei,
  });
...
export async function refundIncomplete(client: any, id: number) {
  const hash = await client.writeContract({
    address: address(CONTRACT_ADDRESS),
    functionName: "refund_incomplete",
    args: [id],
  });
  return confirmed(client, hash);
}
...
export async function createSourceReport(
  ...
  const hash = await client.writeContract({
    address: address(CONTRACT_ADDRESS),
    functionName: "create_source_report",
...
export async function readSourceReport(id: number) {
  return readClient().readContract({
    address: address(CONTRACT_ADDRESS),
    functionName: "get_source_report",
    args: [id],
  });
}
...
export async function listSourceReportIds() {
  return readClient().readContract({
    address: address(CONTRACT_ADDRESS),
    functionName: "list_source_report_ids",
    args: [],
  });
}
