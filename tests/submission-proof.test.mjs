...
test("FactForge has symmetric escrow and recoverable fail-closed settlement", () => {
  for (const signal of ["@gl.public.write.payable", "def create_challenge", "def accept_challenge", "def submit_evidence", "def resolve_challenge", "Both parties must submit at least one evidence URL", "Challenger stake must exactly match proposer stake", "Evidence window is still open", "def refund_unaccepted", "def refund_incomplete", "ChallengeStatus.UNDETERMINED", "proposer_stake = u256(0)", "challenger_stake = u256(0)", "EVIDENCE_BUDGET_PER_SIDE", "emit_transfer"]) assert.ok(source.includes(signal), signal);
  assert.equal(source.includes("ohallenger_won"), false);
});
